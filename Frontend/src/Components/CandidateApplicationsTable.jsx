...
function CandidateApplicationsTable() {
  ...
  useEffect(() => {
    async function fetchJobs(){
      try {
        ...
        const response = await axiosInstance.get(`/jobs/candidate/${userData?.email}`, {
          headers: {
  ...
  return (
    ...
    <table className="w-full text-left">
          ...
          <tbody>
            {jobs.map((job) => (
              <tr key={job?._id} className="border-b border-gray-800 hover:bg-gray-800/50">
                <td className="px-4 py-4">{job?.jobTitle}</td>
                <td className="px-4 py-4">{job?.employmentType}</td>
                <td className="px-4 py-4">{job?.location}</td>
                <td className="px-4 py-4">{new Date(job?.appliedAt).toLocaleDateString()}</td>
