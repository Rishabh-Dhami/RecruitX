  ...
  return (
    <div className='w-full px-20 min-h-[70vh]' >
      ...
      <div className='flex justify-between items-start mt-10'>
        <p className='w-[40%]'>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Vel veniam et eum nulla obcaecati repudiandae minima iste in quia numquam.</p>
        <Link to={"/all-jobs"} className='bg-[#4199c9] py-2 px-8 rounded-lg'>Find More Job</Link>
      </div>
      <div className='w-full flex justify-between flex-wrap gap-y-4 items-center mt-10'>
        {jobs.map((item, index) => (
          <div key={index} className='w-[32%] p-8 bg-[#333333] hover:bg-[#408BB1] rounded-xl'>
          <div>
          <p className='text-red-500 font-semibold text-2xl'>{item.companyName}</p>
          <p className='text-[12px] font-extralight text-gray-300'>{item.location}</p>
          </div>
          <div className='my-4'>
            <h5>{item.jobTitle}</h5>
