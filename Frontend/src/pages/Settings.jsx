    ...
    <div className='w-full min-h-[100vh] py-4 px-6 sm:px-8 lg:px-20 '>
        ...
        <ul className='flex items-center justify-start gap-5 text-xl pt-8 pb-4 mb-8 border-b-2 border-gray-700 cursor-pointer'>
            <li onClick={
              ...
              Profile
              {activeTab === "Profile" ? <p className='h-1 w-[50%] bg-[#4D9ECB]  rounded-md absolute bottom-[-4px]'></p> : null}
            </li>
            <li onClick={() => {
              setActiveTab("Security");
              }} className='relative' >
              Security
             {activeTab === "Security" ? <p className='h-1 w-[50%] bg-[#4D9ECB]  rounded-md absolute bottom-[-4px]'></p> : null}
            </li>
        </ul>
        {activeTab === "Profile" && (<div ref={profileRef} className='mt-2 w-full h-full'>
          <Profile/>
        </div>)}
        {activeTab === "Security" && (<div ref={securityRef} className='mt-2 w-full h-full'>
          <Security/>
        </div>)}
    </div>
  )
